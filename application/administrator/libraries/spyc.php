<?php
	...
	class Spyc
	{
		 ...
		 */
		function YAMLLoad($input, $custom_path = FALSE)
		{
			$spyc = new Spyc;
			return $spyc->load($input, $custom_path);
		}
		 ...
		 * @return array
		 * @param string $input Path of YAML file or string containing YAML
		 * Geshan: Edited function to make custom path possible
		 */
		function load($input, $custom_path)
		{
			// See what type of input we're talking about
			// If it's not a file, assume it's a string

			// print "<br/>Custom path is load spyc:<br>";
			// print $custom_path;
			...
			if ($custom_path)
			{
				$input = preg_replace($bad, $good, $input) . '.yml';
				//print "<br>Am I here";
			}
			else
			{
				$input = APPPATH . preg_replace($bad, $good, $input) . '.yml';
				//print "<br>damn";
			}
		...
		function _toType($value)
		{
			...
			elseif (in_array(strtolower($value), array(
				'true',
				'on',
				'+',
				'yes',
